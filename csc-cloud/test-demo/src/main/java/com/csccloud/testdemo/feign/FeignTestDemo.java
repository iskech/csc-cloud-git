 ...
 */
@FeignClient(name = "test-demo2",fallback = FallBackTemplate.class)
public interface FeignTestDemo {
    @RequestMapping(value = "/api/test2/queryone", method = RequestMethod.POST)
    @ResponseBody
    User getUserByName(@RequestParam("name") String name);
    @RequestMapping(value = "/api/test2/feign", method = RequestMethod.GET)
    @ResponseBody
    User testFeign(@RequestParam("name") String name);
    @RequestMapping(value = "/api/test2/feignFallback", method = RequestMethod.GET)
    @ResponseBody
    User testFeignFallback(@RequestParam("name") String name);
}
